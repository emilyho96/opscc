...
tert_med <- subset(temp_filt, GARD > 46.086 & GARD < 65.014)
tert_high <- temp_filt[temp_filt$GARD >65.014, ]
...
# Fig 1
```{python joint plot}
# this runs better in python itself
...
```{r - prep dataset for AUC analysis}
...
options(datadist='dd')
f<-cph(Surv(Time_OS, Event_OS) ~ RSI,
       data = temp_filt2, surv=T, x=T, y=T)

temp_filt2$lp <- f[["linear.predictors"]]

timeROC_rsi <- timeROC(T=temp_filt2$Time_OS, delta=temp_filt2$Event_OS, marker=temp_filt2$lp,
                  cause=1,weighting="marginal",
                  times=seq(0,100,1), 
                  iid=TRUE)

plot(timeROC_rsi, time= 36)

t <- confint(timeROC_rsi)
t[["CI_AUC"]][31,]
```
...
``` {r GARD}
temp_filt2$GARDneg = temp_filt2$GARD*(-1)
 
dd <- datadist(temp_filt2)
options(datadist='dd')
f<-cph(Surv(Time_OS, Event_OS) ~ GARDneg,
       data = temp_filt2, surv=T, x=T, y=T)

temp_filt2$lp <- f[["linear.predictors"]]

timeROC_gard<-timeROC(T=temp_filt2$Time_OS, delta=temp_filt2$Event_OS, marker=temp_filt2$lp,
                  cause=1,weighting="marginal",
                  times=seq(0,100,1),
                  iid=TRUE)
...
timeROC_clusters<-timeROC(T=temp_filt2$Time_OS, delta=temp_filt2$Event_OS, marker=temp_filt2$lp,
                  cause=1,weighting="marginal",
                  times=seq(0,100,1), 
                  iid=TRUE)
...
f<-cph(Surv(Time_OS, Event_OS) ~ Tstage + Nstage + Pack_years + RSI + Clusters,
       data = temp_filt2, surv=T, x=T, y=T)
