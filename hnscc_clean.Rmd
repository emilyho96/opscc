...
```{r - UVA not in other parts of this file}
...
midpt_filt = median(temp_filt$GARD)

# overall KM - delete
km_os_filt <- ggsurvplot(fit=survfit(Surv(Time_OS, Event_OS) ~ 1, data=temp_filt),
...
cut_os_filt <- ggsurvplot(gard_fit, 
           ...
           xlim = c(0, 60), 
           break.time.by = 12, 
           legend.labs = c("GARD > 64.2", "GARD < 64.2"),
           legend = c(.3,.25),
           pval.size = 7,
           font.legend = 18,
           font.x = 18,
...
cutpoints_top <- cutpoints_filtered_os %>%
  filter(min_n >= 10) %>% #require at least min_n samples in each group
...
# pdf(file="tertiles_filtered.pdf")
# filt_os_opt
# dev.off()
...
```{r - create/print nomogram}
dd <- datadist(temp_filt2)
options(datadist='dd')
f<-cph(Surv(Time_OS, Event_OS) ~ AJCC8 + ECOG + Pack_years + GARD,
...
# ss<-c(0.7, 0.8, 0.9, 0.95, 0.98, 0.99)
# 
# nom <- nomogram(f, 
#                 fun = list(surv3), 
#                 funlabel = c('3 yr surv'), 
#                 fun.at = list(ss) )
# plot(nom, xfrac = 0.4)
# 
# x = f[["linear.predictors"]]
# quantile(x)
```
## Fig 3b
```{r AJCC8}
...
options(datadist='dd')
f<-cph(Surv(Time_OS, Event_OS) ~ AJCC8,
       data = temp_filt, surv=T, x=T, y=T)

temp_filt$lp <- f[["linear.predictors"]]

timeROC_tnm<-timeROC(T=temp_filt$Time_OS, delta=temp_filt$Event_OS, marker=temp_filt$lp,
                  cause=1,weighting="marginal",
                  times=seq(0,100,1), 
                  iid=TRUE)

plot(timeROC_tnm, time= 36)

t <- confint(timeROC_tnm)
t[["CI_AUC"]][31,]
```
...
```{r combined AJCC/ECOG/smoking/clusters}
dd <- datadist(temp_filt2)
options(datadist='dd')
f<-cph(Surv(Time_OS, Event_OS) ~ AJCC8 + ECOG + Pack_years + Clusters,
       data = temp_filt2, surv=T, x=T, y=T)

temp_filt2$lp <- f[["linear.predictors"]]

timeROC_nomo<-timeROC(T=temp_filt2$Time_OS, delta=temp_filt2$Event_OS, marker=temp_filt2$lp,
                  cause=1,weighting="marginal",
                  times=seq(0,100,1), 
                  iid=TRUE)

plot(timeROC_nomo, time= 36)

t <- confint(timeROC_nomo)
t[["CI_AUC"]][31,]
```
...
```{r combined AJCC/ECOG/Smoking/GARD}
dd <- datadist(temp_filt2)
options(datadist='dd')
...
timeROC_withgard<-timeROC(T=temp_filt2$Time_OS, delta=temp_filt2$Event_OS, marker=temp_filt2$lp,
                  cause=1,weighting="marginal",
                  times=seq(0,100,1), 
                  iid=TRUE)

plot(timeROC_withgard, time= 36)

t <- confint(timeROC_withgard)
t[["CI_AUC"]][31,]
```
...
withgard_x <- timeROC_withgard$FP[,which(timeROC_withgard$times==time)]
withgard_y <- timeROC_withgard$TP[,which(timeROC_withgard$times==time)]
leg3 = paste("RTOG + GARD, AUC =",
             toString(round(timeROC_withgard$AUC[which(timeROC_withgard$times==time)]*100,1)))
...
lines(withgard_x, withgard_y, col='#000000', type="l")
...
df = pd.read_csv('GARD_HPVpos6.csv')
...
print('Average # de-escalated in group 2: '+str(np.mean(deesc)))
```
